...
{
  char dot_vscode_path[255];
  char src_folder_path[255];
  char app_folder_path[255];

  sprintf(dot_vscode_path, "%s/.vscode", project_name);
  sprintf(src_folder_path, "%s/src", project_name);
  sprintf(app_folder_path, "%s/src", src_folder_path);

  _mkdir(project_name);
  _mkdir(dot_vscode_path);
  _mkdir(src_folder_path);
  _mkdir(app_folder_path);
  ...
  for (unsigned int i = 0; i < 22; i++)
  {
    ...
    {
      case 0: {
        char tsconfig_app_json[255];
        char tsconfig_app_json_path[255];
        sprintf(tsconfig_app_json_path, "%s/tsconfig.app.json", project_name);

        files[i] = fopen(tsconfig_app_json_path, "w");
        sprintf(tsconfig_app_json, "{\n  \"extends\": \"./tsconfig.json\",\n  \"compilerOptions\": {\n    \"outDir\": \"./out-tsc/app\",\n    \"types\": []\n  },\n  \"files\": [\n    \"src/main.ts\"\n  ],\n  \"include\": [\n    \"src/**/*.d.ts\"\n  ]\n}", project_name);
        fprintf(files[i], tsconfig_app_json);
        fclose(files[i]);
        break;
      }
      case 1: {
        char tsconfig_json[1024];
        char tsconfig_json_path[255];
        sprintf(tsconfig_json_path, "%s/tsconfig.json", project_name);

        files[i] = fopen(tsconfig_json_path, "w");
        sprintf(tsconfig_json, "{\n  \"compileOnSave\": false,\n  \"compilerOptions\": {\n    \"baseUrl\": \"./\",\n    \"outDir\": \"./dist/out-tsc\",\n    \"forceConsistentCasingInFileNames\": true,\n    \"strict\": true,\n    \"noImplicitOverride\": true,\n    \"noPropertyAccessFromIndexSignature\": true,\n    \"noImplicitReturns\": true,\n    \"noFallthroughCasesInSwitch\": true,\n    \"sourceMap\": true,\n    \"declaration\": false,\n    \"downlevelIteration\": true,\n    \"experimentalDecorators\": true,\n    \"moduleResolution\": \"node\",\n    \"importHelpers\": true,\n    \"target\": \"ES2022\",\n    \"module\": \"ES2022\",\n    \"useDefineForClassFields\": false,\n    \"lib\": [\n      \"ES2022\",\n      \"dom\"\n    ]\n  },\n  \"angularCompilerOptions\": {\n    \"enableI18nLegacyMessageIdFormat\": false,\n    \"strictInjectionParameters\": true,\n    \"strictInputAccessModifiers\": true,\n    \"strictTemplates\": true\n  }\n}", project_name);
        fprintf(files[i], tsconfig_json);
        fclose(files[i]);
        break;
      }
      case 2: {
        char tsconfig_spec_json[512];
        char tsconfig_spec_json_path[255];
        sprintf(tsconfig_spec_json_path, "%s/tsconfig.spec.json", project_name);

        files[i] = fopen(tsconfig_spec_json_path, "w");
        sprintf(tsconfig_spec_json, "{\n  \"extends\": \"./tsconfig.json\",\n  \"compilerOptions\": {\n    \"outDir\": \"./out-tsc/spec\",\n    \"types\": [\n      \"jasmine\"\n    ]\n  },\n  \"include\": [\n    \"src/**/*.spec.ts\",\n    \"src/**/*.d.ts\"\n  ]\n}", project_name);
        fprintf(files[i], tsconfig_spec_json);
        fclose(files[i]);
        break;
      }
      case 3: {
        char package_json[2048];
      ...
      case 4: {
        const char index_html[285] = "<!DOCTYPE html>\n<html lang=\"en\">\n  <head>\n    <meta charset=\"UTF-8\">\n    <title>App</title>\n    <base href=\"/\" />\n    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\" />\n    <link rel=\"icon\" href=\"#\" />\n  </head>\n  <body>\n    <app-root></app-root>\n  </body>\n</html>";
        char index_html_path[255];
        sprintf(index_html_path, "%s/index.html", src_folder_path);

        files[i] = fopen(index_html_path, "w");
        fprintf(files[i], index_html);
        fclose(files[i]);
        break;
      }
      case 5: {
        const char main_ts[217] = "import { platformBrowserDynamic } from '@angular/platform-browser-dynamic';\n\nimport { AppModule } from './app/app.module';\n\nplatformBrowserDynamic().bootstrapModule(AppModule)\n  .catch(error => console.error(error));";
        char main_ts_path[255];
        sprintf(main_ts_path, "%s/main.ts", src_folder_path);

        files[i] = fopen(main_ts_path, "w");
        fprintf(files[i], main_ts);
        fclose(files[i]);
        break;
      }
      case 6: {
        const char styles_sass[25] = "// Add your styles here!";
        char styles_sass_path[255];
        sprintf(styles_sass_path, "%s/styles.sass", src_folder_path);

        files[i] = fopen(styles_sass_path, "w");
        fprintf(files[i], styles_sass);
        fclose(files[i]);
        break;
      }
      case 7: {
        const char gitignore[377] = "/dist\n/tmp\n/out-tsc\n/bazel-out\n\n/node_modules\nnpm-debug.log\nyarn-error.log\n\n.idea/\n.project\n.classpath\n.c9/\n*.launch\n.settings/\n*.sublime-workspace\n\n.vscode/*\n!.vscode/settings.json\n!.vscode/tasks.json\n!.vscode/launch.json\n!.vscode/extensions.json\n.history/*\n\n/.angular/cache\n.sass-cache/\n/connect.lock\n/coverage\n/libpeerconnection.log\ntestem.log\n/typings\n\n.DS_Store\nThumbs.db";
